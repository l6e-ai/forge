...
@app.command()
def upsert(
    namespace: str = typer.Option(..., "--ns", "--namespace", help="Memory namespace (e.g., agent name)"),
    collection: str | None = typer.Option(None, "--collection", "-c", help="Optional collection name to target (overrides backend default)"),
    ...
    try:
        payload: dict[str, object] = {"namespace": namespace, "key": key, "content": content}
        if collection:
...
def search(
    ...
    try:
        payload: dict[str, object] = {"namespace": namespace, "query": query, "limit": limit}
        if collection:
            payload["collection"] = collection
        resp = httpx.post(url, json=payload, timeout=10.0)
        resp.raise_for_status()
        ...
        for idx, r in enumerate(results, start=1):
            rprint(f"[cyan]{idx}[/cyan] score={r.get('score', 0):.3f} key={r.get('key','')}\n  {r.get('content','')}")
    except Exception as exc:  # noqa: BLE001
